	...
	$(document).ready(function() {
		...
		*/
		function loadImage() {
			...
			/*
				...
				It also hides the image by applying the "otherImg" CSS class to it.
		...
		*/
		function imageLoaded() {
			...
			if (loadedImages == totalFrames) {
				// ...if so, it makes the first image in the sequence to be visible by removing the "otherImg" class and applying the "currentImg" on it
		...
		*/
		function showThreesixty () {
			...
			$("#capitolmain").fadeIn("slow");
			// Sets the "ready" variable to true, so the app now reacts to user interaction 
			ready = true;
			// Sets the endFrame to an initial value...
			
			if (currentFrame == -66) { //if they're the same
				endFrame = -33; //set current frame to something else
			} else  {  //otherwise
			 	endFrame = -66; //set equal to -60, which is perfect placement
			}
		...
		*/
		function hidePreviousFrame() {
			/*
				Replaces the "currentImg" class with the "otherImg" one on the image.
		...
		*/
		...
		//bind "capitolLabels" mousedown to spinning function
